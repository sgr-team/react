...
import { HandlerEnvironmentPlugin } from "./HandlerEnvironmentPlugin";
...
export class HandlerFactory<Env extends HandlerEnvironment = HandlerEnvironment> {
  ...
  plugin<Delta>(plugin: HandlerEnvironmentPlugin<Env, Delta>): HandlerFactory<Env & Delta> {
    this.plugins.push(plugin);

    return this as unknown as HandlerFactory<Env & Delta>;
  }
  ...
  error(error: unknown) {
    ...
    console.error("HandlerFactory.error: error is not an ApiError", error);
    return new NextResponse("Internal Server Error", { status: 500 });
  }
}
